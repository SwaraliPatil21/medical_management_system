...
def call_addself():
    root.destroy()
    os.system('python3 test_purchase.py')
    ...
    if op>0:
        ...
        try:
            ...
            for i in range(0, allmed_len, 1):
                #mycur.execute(sql,(10,5,8,'2021-05-13', 10, 550))
                mycur.execute(sql,( pur_med[i][0],pur_med[i][1],pur_med[i][2],pur_med[i][3],pur_med[i][4],pur_med[i][5],pur_med[i][6]))
            db.commit()
            print(mycur.rowcount, "record inserted.")
...
welcome()
#=========================Buttons======================
btn1 = Button(frame_right, text='Add Medicines', font='arial 11 bold', width=15, bg="#50aba5", border=2, cursor="hand2", command=additm)
btn1.place(x=85, y=320)
btn2 = Button(frame_right, text='Generate Bill', font='arial 11 bold', padx=10, bg="#50aba5", border=2, cursor="hand2", command=gbill)
btn2.place(x=25, y=360)
btn3 = Button(frame_right, text='Delete Bill', font='arial 11 bold', padx=10, bg="#50aba5", border=2, cursor="hand2", command=clear)
